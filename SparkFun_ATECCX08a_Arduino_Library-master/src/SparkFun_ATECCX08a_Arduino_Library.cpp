...
boolean ATECCX08A::loadPublicKey(uint8_t *data)
{
	uint8_t public_x[32];
	uint8_t public_y[32];
	for(int i =0; i <32; i++)
	...
	Serial.println("Loaded Public Key X Component Successfully");
	...
	if(checkCrc() == false) return false;
	if(inputBuffer[1] == 0x00) {
	Serial.println("Loaded Public Key Y Component Successfully");
	return true;   // If we hear a "0x00", that means it had a successful write
	}
	else return false;
}
